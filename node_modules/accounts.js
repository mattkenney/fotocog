...
var accounts = module.exports =
{
    ...
    {
        var key = 'user/facebook/' + encode.encode_segment(profile.username);
        redis.hgetall(key, function (err, user)
        {
            if (err) { callback (err); return; }
            if (user && user.handle)
    ...
    setUserProperty: function (username, propname, propvalue, callback)
    {
        redis.hset(username2key(username), propname, propvalue, function (err)
        {
            if (err) { callback(err); return; }
            callback();
        });
