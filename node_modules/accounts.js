...
var accounts = module.exports =
{
    ...
    deserializeUser: function (username, callback)
    {
        redis.hgetall(username2key(username), function (err, user)
        {
            if (user)
            {
    ...
    createUser: function (username, password, name, callback)
    {
        encryptPassword(password, null, function (err, value)
        {
            ...
            redis.incr('handle/' + handle, function (err, count)
            {
                ...
                redis.hmset(username2key(username), user, function (err)
                {
                    if (err) { callback(err); return; }
                    user.username = username;
                    callback(null, user);
    ...
    {
        redis.hgetall(username2key(username), function (err, value)
        {
            if (err) { callback(err); return; }
