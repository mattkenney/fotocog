    ...
    days: function (handle, year, month, done)
    {
        ...
        client.get(path).on('response', function (response)
        {
            ...
            response.on('end', function ()
            {
                ...
                {
                    var data = [],
                        prefixes = result && result.ListBucketResult && result.ListBucketResult.CommonPrefixes;
                    for (var i = 0; prefixes && i < prefixes.length; i++)
                    {
                        if ((/\/([0-9]{2})\/$/).test(prefixes[i].Prefix[0]))
                        {
    ...
    photos: function (handle, year, month, day, done)
    {
        ...
        client.get(path).on('response', function (response)
        {
            ...
            response.on('end', function ()
            {
                ...
                {
                    var data = [],
                        contents = result && result.ListBucketResult && result.ListBucketResult.Contents;
                    for (var i = 0; contents && i < contents.length; i++)
                    {
                        data.push(client.signedUrl('/' + contents[i].Key[0], expires));
                    }
                    done(null, data);
                });
            });
        ...
        client.get(path).on('response', function (response)
        {
            ...
            response.on('end', function ()
            {
                ...
                parser.parseString(data, function (err, result)
                {
                    ...
                    for (var i = 0; prefixes && i < prefixes.length; i++)
                    {
                        if ((/\/([0-9]{4})\/$/).test(prefixes[i].Prefix[0]))
                        {
