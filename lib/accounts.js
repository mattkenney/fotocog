...
var accounts = module.exports =
{
    ...
    createUser: function (username, password, name, callback)
    {
        encryptPassword(password, null, function (err, value)
        {
            ...
            db.incr('handle/' + handle, function (err, count)
            {
                if (err) { callback(err); return; }
                if ((0|count) > 1)
                {
    ...
    {
        db.hGet(username2key(username), propname, callback)
    },
    ...
    setPassword: function (username, password, callback)
    {
        ...
        {
            if (err) { callback(err); return; }
            db.hSet(username2key(username), "password", hash, function (err)
            {
                if (err) { callback(err); return; }
                callback();
            });
    ...
    {
        db.hSet(key, propname, propvalue, function (err)
        {
            if (err) { callback(err); return; }
            callback();
        });
